"""
ACL论文数据集挖掘主程序
从ACL Anthology获取2025年Main和Findings论文，提取数据集信息
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('dataset_miner_acl.log', encoding='utf-8'),
...
class ACLDatasetMiner:
    """ACL数据集信息挖掘器"""
    
    def __init__(self, output_file: str = "outputs/dataset_acl_results.jsonl"):
        """
        初始化
    ...
    def run(self, year: int = 2025):
        ...
        Args:
            year: ACL年份
        """
        logger.info(f"=== 开始挖掘ACL {year}数据集信息 ===")
        logger.info(f"目标: Main和Findings论文")
        
        # 初始化下载器
        downloader = ACLDownloader(year=year, temp_dir="temp")
        ...
        try:
            ...
            for pdf_path, category, paper_info in downloader.download_and_process_papers():
                ...
                logger.info(f"Anthology ID: {paper_info.get('anthology_id', 'N/A')}")
                logger.info(f"{'='*60}")
