...
from downloader import ICLRDownloader
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('dataset_miner.log', encoding='utf-8'),
...
class DatasetMiner:
    ...
    def __init__(self, output_file: str = "outputs/dataset_iclr_results.jsonl"):
        """
        初始化
        ...
        try:
            ...
            if not paper_meta:
                ...
                paper_meta = {
                    ...
                    "authors": [],
                    "venue": f"ICLR {category}",
                    "year": "2025",
                    "url": paper_info.get('openreview_url', ''),
                    "is_fellow": "false"
                }
            
            # 补充OpenReview URL
            if not paper_meta.get('url'):
                paper_meta['url'] = paper_info.get('openreview_url', '')
    ...
    def run(self, year: int = 2025):
        ...
        Args:
            year: ICLR年份
        """
        logger.info(f"=== 开始挖掘ICLR {year}数据集信息 ===")
        
        # 初始化下载器
        downloader = ICLRDownloader(year=year, temp_dir="temp")
        ...
        try:
            ...
            for pdf_path, category, paper_info in downloader.download_and_process_papers():
                ...
                logger.info(f"\n{'='*60}")
                logger.info(f"处理第 {total_papers} 篇论文 [{category}]")
...
def main():
    """主函数"""
    miner = DatasetMiner(output_file="outputs/dataset_iclr_results.jsonl")
    miner.run(year=2025)
