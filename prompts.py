...
请提取以下信息：
1. 论文标题 (title)
2. 作者列表 (authors)，格式为 [{{"name": "作者名", "institution": "机构"}}]
3. 会议或期刊名称 (venue)
4. 发表年份 (year)
5. 论文URL (url)，如果文本中没有明确的论文URL，可以留空
6. is_fellow: 固定填写 "false"
...
请提取以下信息：
1. content: 数据集的详细描述（用途、规模、特点等）
2. type: 数据集类型，例如 ["multiple-choice questions", "image classification", "text generation"]
3. domain: 数据集涉及的领域，例如 ["natural language processing", "computer vision", "audio"]
4. fields: 应用方向，例如 ["large language models evaluation", "object detection"]
5. dataset_link: 数据集的下载链接（从提供的URL中选择最相关的，或根据上下文推断）
6. platform: 平台名称，例如 "GitHub", "HuggingFace", "Kaggle", "官方网站"等
